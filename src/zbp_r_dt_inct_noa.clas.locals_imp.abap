CLASS lhc_Incident DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_history_index EXPORTING ev_incuuid      TYPE sysuuid_x16
                              RETURNING VALUE(rv_index) TYPE zde_his_id.
...
** Read root entity entries
    READ ENTITIES OF zdd_incident_noa IN LOCAL MODE
     ENTITY Incident
...
** This important for logic
    DELETE incidents WHERE creation_date IS NOT INITIAL.

    CHECK incidents IS NOT INITIAL.

** Get Last index from Incidents
    SELECT FROM zdt_inct_noa
...
** Modify status in Root Entity
    MODIFY ENTITIES OF zdd_incident_noa IN LOCAL MODE
      ENTITY Incident
      UPDATE
      FIELDS ( incident_id
               creation_date
...
** Disable changeStatus for Incidents Creation
    ...
    IF lv_create_action EQ 1.
      lv_history_index = get_history_index( IMPORTING ev_incuuid = incidents[ 1 ]-inc_uuid ).
    ELSE.
      lv_history_index = 1.
    ENDIF.
...
* Declaration of necessary variables
    DATA: lt_updated_root_entity TYPE TABLE FOR UPDATE zdd_incident_noa,
          lt_association_entity  TYPE TABLE FOR CREATE zdd_incident_noa\_History,
          lv_status              TYPE zde_status_code,
          lv_text                TYPE zde_text,
          lv_exception           TYPE string,
          lv_error               TYPE c,
          ls_incident_history    TYPE zdt_inct_h_noa,
          lv_max_his_id          TYPE zde_his_id,
          lv_wrong_status        TYPE zde_status_code.
...
* Customize error messages
        APPEND VALUE #( %tky = <incident>-%tky
                        %msg = NEW zcl_incident_messages_noa( textid = zcl_incident_messages_noa=>status_invalid
...
** Get parameters
    LOOP AT incidents ASSIGNING FIELD-SYMBOL(<incident>).
      lv_max_his_id = get_history_index( IMPORTING ev_incuuid = <incident>-inc_uuid ).
